...
Mindlin 中厚板屈曲问题研究的是：板在面内压力作用下，当荷载逐渐增大至某一临界值时，原始的平面内平衡状态失去稳定性，板突然发生横向挠度（屈曲分岔）。与强度失效不同，屈曲失效通常发生在应力远低于材料屈服强度时，具有突发性。Mindlin 板理论考虑横向剪切变形，适用于中厚板（$1/100 \leq h/L \leq 1/5$），其核心假设是变形后横截面仍保持平面但不再垂直于中面。

考虑 Mindlin 中厚板的屈曲问题，求解目标为临界荷载系数 $\lambda$ 和屈曲模态，对应的广义特征值问题为：
\begin{equation}
...
Mindlin 理论假设：变形前垂直于中面的直线纤维，在变形后仍保持直线，但不一定保持垂直于中面。考虑完整的三维位移场：
\begin{equation}
\begin{cases}
\bar{u}_\alpha(x_1,x_2,x_3) = u_\alpha(x_1,x_2) - x_3\varphi_\alpha(x_1,x_2) \\
\bar{u}_3(x_1,x_2,x_3) = w(x_1,x_2)
\end{cases}
\end{equation}
其中，$u_\alpha$ 为中面面内位移，$w$ 为横向挠度，$\varphi_\alpha$ 为法线纤维的转角。

屈曲分析需考虑几何非线性，采用完整的 Green-Lagrange 应变张量：
\begin{equation}
...
\textbf{第一项：面内应变项}。将总应力 $\sigma_{\alpha\beta}$ 拆分为预应力度与屈曲增量应力：
\begin{equation}
\sigma_{\alpha\beta} = \bar{\sigma}_{\alpha\beta} + \sigma_{\alpha\beta}^{inc}
\end{equation}
其中，预应力度 $\bar{\sigma}_{\alpha\beta}$ 沿厚度均匀分布（与 $x_3$ 无关）；增量应力 $\sigma_{\alpha\beta}^{inc}$ 随厚度弹性变化。将应变展开式代入并完全展开，得到 8 个相乘项。依据小变形假设和屈曲分岔点的特性：第 1、2 项涉及膜应变，其一阶变分为零；第 6、8 项为增量应力与变形梯度的乘积，属三阶小量舍去；第 3 项因预应力度沿厚度均匀分布与 $x_3$ 无关，$\int x_3 dx_3 = 0$ 恒消去。保留第 4、5、7 项，对厚度积分后定义弯矩合力：
\begin{equation}
...
\textbf{第二项：横向剪切项}。对于纯面内加载的板，预屈曲横向剪应力 $\bar{\sigma}_{\alpha 3} = 0$，故 $\sigma_{\alpha 3} = \sigma_{\alpha 3}^{inc}$。由虎克定律，引入剪切修正系数 $\kappa$ 后：
\begin{equation}
...
\textbf{第三项}：厚度方向法向应变项，在平面应力假设 $\sigma_{33}=0$ 下贡献为零。
...
\end{aligned}
\end{equation}
其中前两项为弹性内力虚功（弯曲与剪切），后两项为预应力度对应的几何刚度虚功（横向挠度与转角各一项）。当板发生屈曲时，预加压应力在屈曲位移梯度上做负功，等效于降低系统的有效刚度；当几何刚度项的绝对值等于弹性刚度项时，系统总刚度矩阵奇异，达到临界屈曲状态。



\section{弱形式}

对屈曲虚功方程各变量取变分并令其为零，将离散场代入后得到离散形式的广义特征值方程。形函数内插为：
\begin{equation}
...
面外材料刚度矩阵 $k_b$ 由弯曲刚度（Bending）与横向剪切刚度（Shear）共同组成。对应自由度 $[w,\varphi_1,\varphi_2]^T$，展开后的对称矩阵形式如下：
\begin{equation}
k_b = \begin{bmatrix}
...
\textbf{$k_{\varphi_\alpha\varphi_\gamma}$}：
...
\textbf{$k_g^{\varphi_\alpha\varphi_\gamma}$}：
\begin{equation}
...
将上述所有子块合併，针对中厚板面外屈曲模式，最终的完整展开广义特征值矩阵方程呈现如下：
\begin{equation}
\begin{pmatrix}
k_{ww} & k_{w\varphi_1} & k_{w\varphi_2} \\
k_{\varphi_1 w} & k_{\varphi_1\varphi_1} & k_{\varphi_1\varphi_2} \\
...
\textbf{格式稳定性分析}：屈曲问题中，几何刚度矩阵 $\mathbf{K}_G$ 对应于预压应力状态为半负定。为保证屈曲前系统总刚度矩阵 $(\mathbf{K} + \lambda\mathbf{K}_G)$ 的正定性，混合离散格式需满足 LBB稳定性条件。本文采用的剪切应力场插值阶数经约束比分析验证，可保证 inf-sup 常数有正下界，从而在薄板极限下避免剪切自锁并维持刚度矩阵的正定性。当 $\lambda$ 增大至临界值时，总刚度矩阵首次出现奇异，对应第一阶屈曲模态。
